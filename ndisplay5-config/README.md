...
Place the **Red Komodo** cine camera actor in the **World Outliner** to be a child of **LSUnDisplayConfig**.  This way when we move the entire panel the cine camera moves with it.  Make sure the **CineCamera** is at `0,0,0` in both **Transform | Locations** and **Transform | Rotation** so we do not offset it.  The position will be given to the camera by the **OptiTrack** system.
